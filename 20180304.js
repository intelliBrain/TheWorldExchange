...
function loadAccount(loadOrderbookNext) {
  ...
  new Promise(function(resolve, reject) { 
    ...
    if(address!="") {
      if(changeMiscMenu) {
        $("#miscMenu").html("<div><a href='#started' onclick='$(\"#about\").css(\"display\", \"block\"); jQuery(\"html,body\").animate({scrollTop: jQuery(\"#started\").offset().top}, 1000); setURL(\"#started\"); "+(isMobile()? "hideMiscMenu();":"")+" return false;'>Min "+baseCurrency+": "+nFormatter(minBaseCurrency, accuracy)+"</a> | <a href='#started' onclick='$(\"#about\").css(\"display\", \"block\"); jQuery(\"html,body\").animate({scrollTop: jQuery(\"#started\").offset().top}, 1000); setURL(\"#started\"); "+(isMobile()? "hideMiscMenu();":"")+" return false;'>Current Fee: "+fee.toFixed(6)+"</a><br /><a href='"+accountHistoryURL+""+address+"' onclick='"+(isMobile()? "hideMiscMenu();":"")+"' target='_blank'>View History</a> | <a href='#export' onclick='accountHistoryCSV();' id='exportTrades'>Export Trades</a></div><div><a href='#' onclick='showTrustlines(); "+(isMobile()? "hideMiscMenu();":"")+"'>Set "+(settings["requireAuthorization"]? "Authorized Token Holders":"Receivable Tokens")+"</a></div><!--<div><a href='#started' onclick='$(\"#about\").css(\"display\", \"block\"); jQuery(\"html,body\").animate({scrollTop: jQuery(\"#started\").offset().top}, 1000); setURL(\"#started\"); "+(isMobile()? "hideMiscMenu();":"")+" return false;'>How to Fund / Deposit</a></div>--><div><a href='#advanced' onclick='showSettings();"+(isMobile()? "hideMiscMenu();":"")+"'>Advanced Settings</a></div>"+commonMenu);
    ...
    .then(function (transactions) {
      if(transactions!=null && transactions.length>0 && transactions[0].address == addressRequest && transactions[0].specification!=null && transactions[0].specification.memos[0].data!=null) {
        ...
        if(isSpam(transactions[0].specification.memos[0].data)) {
          console.log("Display name ignored due to potential code: "+transactions[i].specification.memos[0].data);
        }
        else {
...
function getURLRequest(name) {
    ...
    if(results != null) $("#"+name).val(decodeURIComponent(results[1].replace(/\+/g, " ")));
}
